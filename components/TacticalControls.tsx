...
const SLIDERS: { key: keyof Tactics; label: string; lo: string; hi: string }[] = [
  { key: "attack", label: "Attack Level", lo: "Cautious", hi: "All-out" },
  { key: "line", label: "Defensive Line", lo: "Deep", hi: "High" },
  { key: "press", label: "Pressing", lo: "Contain", hi: "Aggressive" },
  { key: "tempo", label: "Tempo", lo: "Slow", hi: "Fast" },
  { key: "width", label: "Width", lo: "Narrow", hi: "Wide" },
];

const TOGGLES: { key: keyof Tactics; label: string }[] = [
...
export default function TacticalControls() {
  ...
  return (
    <div className="glass rounded-2xl p-4">
      <div className="mb-3 text-xs font-semibold uppercase tracking-widest text-white/50">Formation</div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-2">
        {TOGGLES.map((t) => {
          const on = tactics[t.key] as boolean;
          return (
            <button
              key={t.key}
              onClick={() => setTactic(t.key, !on as never)}
