  ...
  return (
    <div className="glass-strong rounded-2xl px-5 py-3 shadow-glow">
      <div className="flex items-center justify-between gap-4">
        <TeamCell name={match.home.name} flag={match.home.flag} color={match.home.primary} align="right" />
        <div className="flex flex-col items-center">
          ...
          <div className="mt-0.5 flex items-center gap-2">
            <span className="chip bg-red-500/20 text-neon-red">
              ...
              {isFT ? "FT" : `${Math.min(snap.minute, 90)}'`}
              {snap.minute > 90 && !isFT ? " ET" : ""}
            </span>
          </div>
        </div>
        <TeamCell name={match.away.name} flag={match.away.flag} color={match.away.primary} align="left" />
      </div>
      <div className="mt-1 text-center text-[11px] uppercase tracking-widest text-white/40">
        {match.year} World Cup · {match.stage} · {match.venue}
      </div>
    </div>
  );
}
